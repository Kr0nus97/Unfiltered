...
import { LogIn, LogOut, User as UserIcon, Users, Info, Mail, KeyRound, UserPlus, Eye, EyeOff, Loader2 } from "lucide-react"; 
      ...
      <div className="bg-card p-6 rounded-lg shadow-md border max-w-lg mx-auto">
        {user && !isGuestMode ? ( 
          <div className="flex flex-col items-center sm:items-start">
            ...
            <Button onClick={signOutUser} variant="destructive" className="w-full sm:w-auto" disabled={loading}>
           ...
           <div className="flex flex-col items-center sm:items-start">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full">
                <Button onClick={() => { signOutUser(); setAuthMode("signin"); }} className="bg-accent text-accent-foreground hover:bg-accent/90 flex-grow" disabled={loading}>
                     {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading...</> : <><LogIn className="mr-2 h-4 w-4" /> Sign In / Create Account</>}
                </Button>
            </div>
          </div>
        ) : ( 
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input id="password-signin" type={showPassword ? "text" : "password"} placeholder="••••••••" {...signInForm.register("password")} className="pl-10 pr-10" />
                     <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8" onClick={togglePasswordVisibility} aria-label={showPassword ? "Hide password" : "Show password"}>
                ...
                <div>
                  <Label htmlFor="confirmPassword-signup">Confirm Password</Label>
                   <div className="relative">
                    ...
                    <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8" onClick={toggleConfirmPasswordVisibility} aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}>
